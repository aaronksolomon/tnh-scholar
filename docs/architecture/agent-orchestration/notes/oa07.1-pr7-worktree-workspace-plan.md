...
PR-7 should update `src/tnh_scholar/agent_orchestration/kernel/service.py:261` and `src/tnh_scholar/agent_orchestration/kernel/service.py:294` so `RUN_AGENT` and `RUN_VALIDATION` execute against the worktree root.
