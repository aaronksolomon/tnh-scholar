...
class YTDownloader:
    ...
    def get_transcript(
        self, url: str, lang: str = "en", output_path: Optional[Path] = None
    ) -> VideoTranscript:
        """Retrieve video transcript with associated metadata."""
        raise NotImplementedError

    def get_audio(self, url: str, start: str, end: str, output_path: Optional[Path]) -> VideoAudio:
    ...
    def get_video(
        self, url: str, quality: Optional[str] = None, output_path: Optional[Path] = None
    ) -> VideoFile:
        """
...
class DLPDownloader(YTDownloader):
    ...
    def get_video(
        ...
        """
        temp_path = Path.cwd() / TEMP_FILENAME_FORMAT
        video_options = self._with_runtime_options(
            DEFAULT_VIDEO_OPTIONS | self.config | {"outtmpl": str(temp_path)}
        )
        if quality:
    ...
    def _convert_filename(self, temp_path: Path, metadata: Metadata, output_path: Optional[Path]) -> Path:
        """
        ...
        if video_id not in str(temp_path):
            raise VideoProcessingError(
                f"Temporary path '{temp_path}' does not contain video ID '{{video_id}}'."
            )
        if not temp_path.suffix:
            raise VideoProcessingError(f"Temporary path '{temp_path}' does not have a file extension.")

        if not output_path:
            new_filename = self.get_default_filename_stem(metadata)
            new_path = Path(str(temp_path).replace(TEMP_FILENAME_STR.format(id=video_id), new_filename))
