...
def transcribe_youtube_videos(urls: list[str], output_base_dir: Path, max_chunk_duration: int = DEFAULT_CHUNK_DURATION_S, start: str = None, translate=False):
    """
    ...
    for url in urls:
        try:
            ...
            try:
                tmp_audio_file.rename(audio_file)  # Move the audio file to the video directory
...
if __name__ == "__main__":
    ...
    group.add_argument(
        "--url", 
        type=str, 
        help="Single YouTube video URL to transcribe."
    )
    parser.add_argument(
        "--output_dir", 
        type=str, 
    ...
    transcribe_youtube_videos(video_urls, output_directory, args.max_chunk_duration, args.start, translate=args.translate)