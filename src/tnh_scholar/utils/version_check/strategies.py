...
def check_version_diff(
    installed: Version, reference: Version, vdiff_matrix: str
    ...
    micro_diff = (
        abs(reference.micro - installed.micro)
        if (reference.major == installed.major and reference.minor == installed.minor)
        else 0
    )

    diff_details = {"major": major_diff, "minor": minor_diff, "micro": micro_diff}
