...
class AudioHandlerConfig(BaseSettings):
    ...
    output_format: Optional[str] = Field(
        default=None, description="Audio output format used when exporting segments (e.g., 'wav', 'mp3')."
    )
    temp_storage_dir: Optional[Path] = Field(
        default=None,
        description="Optional directory path for storing temporary audio files (currently unused).",
    )
    max_segment_length: Optional[int] = Field(
        default=None, description="Maximum allowed segment length (in milliseconds)."
    )
