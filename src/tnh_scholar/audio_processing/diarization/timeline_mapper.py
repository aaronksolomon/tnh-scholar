...
class TimelineMapperConfig(BaseModel):
    ...
    debug_logging: bool = Field(default=False, description="Enable detailed logging of mapping decisions")
    map_speakers: bool = Field(
        default=True, description="Assign speaker to mapped timings using diarization segment speaker."
    )
...
class TimelineMapper:
    ...
    class _TimeUnitMapper:
        ...
        def _map_text_unit(self, unit: TimedTextUnit) -> TimedTextUnit:
            ...
            return self._apply_mapping(unit, best_segment)
        ...
        def _find_overlapping_segments(self, unit: TimedTextUnit) -> List[DiarizedSegment]:
            ...
            return [
                ...
                if (segment.mapped_start <= unit.end_ms and segment.mapped_end >= unit.start_ms)
            ]

        def _choose_best_overlap(
            self, unit: TimedTextUnit, candidates: List[DiarizedSegment]
        ) -> DiarizedSegment:
        ...
        def _calculate_overlap(self, unit: TimedTextUnit, segment: DiarizedSegment) -> int:
        ...
        def _choose_closest_proximal(
            self, unit: TimedTextUnit, before: DiarizedSegment, after: DiarizedSegment
        ) -> DiarizedSegment:
            """
        ...
        def _apply_mapping(self, unit: TimedTextUnit, segment: DiarizedSegment) -> TimedTextUnit:
            """
            ...
            unit = unit.model_copy(update={"start_ms": new_unit_start, "end_ms": new_unit_end})
