...
def safe_yaml_load(yaml_str: str, *, context: str = "unknown") -> dict:
    try:
        ...
        if not isinstance(data, dict):
            logger.warning("YAML in [%s] is not a dict. Returning empty metadata.", context)
            return {}
        return data
...
class Metadata(MutableMapping):
    ...
    def __init__(self, data: Optional[Union[Dict[str, Any], "Metadata"]] = None) -> None:
        self._data: Dict[str, JsonValue] = {}
        if data is not None:
            raw_data = data._data if isinstance(data, Metadata) else data
            processed_data = {k: self._process_value(v) for k, v in raw_data.items()}
    ...
    def _process_value(self, value: Any) -> JsonValue:
        ...
        if not isinstance(value, (str, int, float, bool, list, dict, type(None))):
            raise ValueError(f"Value {value} of type {type(value)} has no conversion to JsonValue.")
        return value
...
class ProcessMetadata(Metadata):
    ...
    def __init__(self, step: str, processor: str, tool: Optional[str] = None, **additional_params):
...
class Frontmatter:
    ...
    def embed(cls, metadata: Metadata, content: str) -> str:
        ...
        return f"{cls.generate(metadata)}{content.strip()}"
