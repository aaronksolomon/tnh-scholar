    ...
    def _get_db_credentials(self):
        ...
        try:
            if not hasattr(st, 'secrets'):
                raise ConfigurationError("Database configuration not found. Please ensure .streamlit/secrets.toml is configured.")
...
class PatternRepository:
    ...
    def add_pattern(self, pattern: 'Pattern') -> str:
        ...
        ids = self.vector_store.add_texts(
            texts=[pattern.instructions],
            metadatas=[pattern_data["metadata"]]
        )
        return ids[0]
...
def main():
    ...
    with st.sidebar:
        ...
        upload_type = st.radio(
            "Upload Type",
            ["Single Pattern", "Git Repository"]
        )

        if upload_type == "Single Pattern":
            if uploaded_file := st.file_uploader(
                "Upload Pattern (.md)", type=["md"]
            ):
    ...
    with tab1:
        if query := st.text_input("Search patterns..."):
            ...
            for result in results:
                with st.expander(f"{result['metadata']['name']} ({result['score']:.2f})"):
    ...
    with tab2:
        ...
        patterns = repo.supabase.table("patterns").select("*").range(
            (page-1)*patterns_per_page,
