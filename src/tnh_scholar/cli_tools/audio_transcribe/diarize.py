# !!! WARNING THIS MODULE IS NOT COMPLETE, IT NEEDS REFACTOR WITH !!!
# !!! DIARIZE_POC3.PY FOR UP TO DATE SPEAKER PROCESSING !!!    
...
from .pyannote_interface import PyannoteClient, PyannoteConfig
...
class ProcessingConfig:
    ...
    # Segment file naming
    FILENAME_TEMPLATE = "{speaker}_{start_ms}_{end_ms}.mp3"
    
    # Chunking parameters
    TARGET_CHUNK_SIZE = 7 * 60  # 7 minutes
    SPACING_THRESHOLD = 10  # seconds
...
class DiarizationResult(BaseModel):
    ...
    def get_segments(self) -> List[DiarizationSegment]:
        """Extract and convert raw segments to DiarizationSegment objects."""
        if self.status != PyannoteConfig.SUCCESS_STATUS:
            return []
            
        raw_segments = self.output.get(PyannoteConfig.DIARIZATION_FIELD, [])
        return [DiarizationSegment(**segment) for segment in raw_segments]
...
class DiarizationProcessor:
    ...
    def __init__(self, 
        ...
        self.target_chunk_size = float(ProcessingConfig.TARGET_CHUNK_SIZE)
        self.spacing_threshold = float(ProcessingConfig.SPACING_THRESHOLD)
    ...
    def _save_json_data(
        self, 
        filename: str, 
        data: List[SpeakerChunk] 
        ) -> None:
        json_path = self.output_dir / filename
        json_data = [obj.model_dump() for obj in data]
        write_str_to_file(json_path, json.dumps(json_data, indent=2), overwrite=True)
    ...
    def diarize(
        ...
        # Process results
        return self.process_diarization_results(results, extract_audio)
