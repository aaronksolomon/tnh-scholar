...
@click.command()
@click.argument("url")
@click.option("-l", "--lang", default="en", help="Language code for transcript (default: en)")
@click.option("-k", "--keep", is_flag=True, help="Keep downloaded datafile: TTML transcript.")
@click.option("-i", "--info", is_flag=True, help="Return only metadata in YAML frontmatter format.")
@click.option("-n", "--no-embed", is_flag=True, help="Do not embed metadata in transcript file.")
@click.option(
    "-o",
    "--output",
    type=click.Path(),
...
def generate_metadata(service: YTDownloadService, url: str, keep: bool, output_path: Optional[Path]) -> None:
...
def generate_transcript(
    service: YTDownloadService, url: str, lang: str, keep: bool, no_embed: bool, output_path: Optional[Path]
) -> None:
    metadata, ttml_path = get_ttml_download(service, url, lang, output_path)

    process_metadata = ProcessMetadata(
        step="generate_transcript", processor="DLPDownloader", tool="ytt-fetch"
    )
    if output_path:
...
def export_ttml_data(
    metadata: Metadata, ttml_path: Optional[Path], no_embed: bool, output_path: Optional[Path], keep: bool
):
    try:
